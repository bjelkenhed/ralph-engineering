---
description: "Start Ralph Wiggum session (single continuous context)"
argument-hint: "[PROMPT] [--max-iterations N] [--completion-promise TEXT] [--prd FILE]"
allowed-tools: ["Bash(${CLAUDE_PLUGIN_ROOT}/scripts/setup-ralph-session.sh *)", "Read", "Write", "Edit", "Bash(git *)", "Bash(pnpm *)"]
hide-from-slash-command-tool: "true"
---

# Ralph Session Command

Execute the setup script to initialize the Ralph session by running this Bash command:

```
"${CLAUDE_PLUGIN_ROOT}/scripts/setup-ralph-session.sh" $ARGUMENTS
```
...
If no PRD exists and no `--prd` option is provided, ralph-session works as before:
- Requires a PROMPT argument
- Iterates until completion promise is output or max iterations reached
