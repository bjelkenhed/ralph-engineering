...
merge_permissions() {
  ...
  if [[ ! -f "$TEMPLATE_FILE" ]]; then
    echo "⚠️  Warning: Permissions template not found: $TEMPLATE_FILE" >&2
    return 0
  fi

  # Check if jq is available
  if ! command -v jq &> /dev/null; then
    echo "⚠️  Warning: jq not installed - cannot merge permissions automatically" >&2
    return 0
  fi

  # Validate template has expected structure
  if ! jq -e '.permissions.allow' "$TEMPLATE_FILE" &>/dev/null; then
    echo "⚠️  Warning: Template missing .permissions.allow - skipping merge" >&2
    return 0
  fi
  ...
  temp_file="$(mktemp)" || {
    echo "⚠️  Warning: Failed to create temp file - skipping merge" >&2
    return 0
  }
  ...
  if ! jq -e '.permissions.allow' "$temp_file" &>/dev/null; then
    echo "⚠️  Warning: Merge produced invalid output - skipping" >&2
    rm -f "$temp_file"
    return 0
  fi
  ...
  mv "$temp_file" "$settings_file" || {
    echo "⚠️  Warning: Failed to write $settings_file" >&2
    rm -f "$temp_file"
    return 0
  }
